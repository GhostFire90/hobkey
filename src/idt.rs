...
extern "x86-interrupt" {
  fn empty_int(stack_frame: InterruptStackFrame);
}
...
pub extern "sysv64" fn IDTR_init(idtr: *mut InterruptDescriptor64)
{
  ...
  {
    unsafe {
      idtr.add(i).write(InterruptDescriptor64::new(
        empty_int as *const () as u64,
        GATE_PRESENT | GATE_INTERRUPT,
      ))
    }
  }
}
...
pub fn set_interrupt(
  vector: u8,
  inttype: u8,
  callback: unsafe extern "x86-interrupt" fn(InterruptStackFrame) -> (),
)
{
  ...
  unsafe {
    idtr
      .add(vector as usize)
      .write(InterruptDescriptor64::new(callback as u64, inttype));
    refresh_idt();
  }
}
