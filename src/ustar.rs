...
pub fn find_file(
  ...
  arc: NonNull<u8>,
  len: usize,
) -> Result<(UstarHeader, usize), UstarError>
{
  ...
  while current_idx < len
  {
    ...
    if header.mag == CORRECT_MAG.to_bytes_with_nul()
    {
      ...
      if CStr::from_bytes_until_nul(&full_name)
        .unwrap()
        .to_str()
        .unwrap()
        == filename
      ...
      current_idx += 512
        + crate::helpers::atou(&header.size_ascii_octal, 8).ok_or(UstarError::IncorrectFormat)?;
    }
    else
    {
