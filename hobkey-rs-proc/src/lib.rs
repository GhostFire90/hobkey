...
pub fn preserve_temp_map(_: TokenStream, item: TokenStream) -> TokenStream
{
  ...
  fn_body.block.stmts.insert(
    0,
    syn::parse(
      quote!(let tmp_mapped : u64 = crate::memory::paging::PageTableManager::get_temp();).into(),
    )
    .unwrap(),
  );
  let mut idx = 0;
  ...
  if indexes.len() != 0
  {
    for i in indexes
    {
      fn_body.block.stmts.insert(
        i,
        syn::parse(
          quote!(crate::memory::paging::PageTableManager::map_temp(tmp_mapped).unwrap();).into(),
        )
        .unwrap(),
      );
    }
